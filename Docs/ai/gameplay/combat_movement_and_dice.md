...
- A lone `1d20` free roll (or a character-sheet `1d20` target) resolves as `unresolved`: two independent d20 values, neither picked by the server, with the same modifier applied to both. An `Xd20` with `X > 1` resolves as `normal`: `X` independent outcomes with no pairing semantics. Advantage/disadvantage stay in the shared engine only for `InitiativeRollModal`'s vestigial client-side path (`initiativeMode` on a token); no server-authoritative caller may request them for a free roll.
- New logs carry additive per-die detail: an id unique within the roll, sides, value, logical group, and `kept`/`discarded`/`unresolved` disposition. The disposition field is retained on every log for backward compatibility, but nothing generates `discarded` any more (it required a client-declared advantage/disadvantage mode, which no longer exists for a free roll) and the presentation no longer renders `kept` and `discarded` with different emphasis. The two independent d20s of a character-sheet or free-roll `1d20` target remain `unresolved` because the reader chooses which applies. A d100 remains one logical die from 1 to 100.
