...
### Conditions, effective speed, and standing up

- When an adventurer moves their own creature directly (not a vehicle), the budget is derived from an **effective speed** (`shared/token-conditions.mjs`'s `effectiveSpeed`), not the raw `movementCells`, floored on every division: `grappled`, `restrained`, `paralyzed`, `petrified`, `stunned`, `unconscious`, or exhaustion level 5+ bring it to 0 (dash and extra movement then add nothing, and the server rejects the move in every session mode — exploration, roll phase, or a started round — naming the condition); exhaustion level 2-4 halves the base speed before dash doubles it and extra movement is added. A vehicle's movement and the master's movement are never affected by this — a vehicle uses its driver's raw `movementCells`, condition-free, exactly as before this capability.
- `pathCost` (`shared/grid-movement.mjs`) takes an optional `stepCostMultiplier` (default 1, so every existing caller is unaffected). When an adventurer moves their own creature and it currently has `prone`, the multiplier is 2: crawling costs double per step, with the diagonal alternation still carried forward exactly as for a normal move. Neither the master's movement nor a vehicle's is ever multiplied this way.
- **Standing up** (`POST /api/battle-map/stand-up`) removes `prone` from a token the caller may modify (owner, including a familiar, or the master). For the master, in any session mode, it is free. For an adventurer: rejected during the roll phase like any movement; rejected outright at effective speed 0, naming the condition; otherwise, when the round has started, it costs `standUpCost(effectiveCells)` (half the effective speed, floored, ignoring dash/extra movement) charged against that round's movement used, and rejected if the remainder is insufficient; in exploration it is free. Removing `prone` this way never re-adds it and never touches `unconscious`.
...
Master actions use full snapshot undo. Adventurer movement, dash, owned-token updates, extra movement, condition changes, and standing up use per-user inverse actions. A user cannot undo another user's action. A condition-change undo reverts only what that specific operation changed (the conditions it added or removed, or the exhaustion level it set), never a condition or level changed by someone else afterward; a stand-up undo re-adds `prone` and returns the movement it charged.
