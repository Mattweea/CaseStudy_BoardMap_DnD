...
| `AUTH_PASSWORD_SALT` | Fastify | Password hashing salt; production must not use the demo default |
| `AUTH_USERS_JSON` | Fastify | Optional non-empty custom authentication user array |
...
Do not commit real credentials or secrets. `AUTH_USERS_JSON` changes authentication records but does not by itself create frontend roster profiles.
...
- Authentication sessions, undo stacks, SSE clients, and live state are process memory.
