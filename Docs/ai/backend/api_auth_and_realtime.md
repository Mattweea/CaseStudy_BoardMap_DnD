...
- The server loads users from `AUTH_USERS_JSON` when it is a valid non-empty array; otherwise it uses the canonical demo roster.
- Passwords are hashed with `scrypt` and `AUTH_PASSWORD_SALT`; hashes and raw passwords are never returned.
...
- Login ensures the canonical adventurer token exists and aligns its roster-owned fields.

## Authorization classes

- Public: `GET /api/health`.
- Session discovery: `GET /api/auth/session`, plus login/logout.
- Authenticated state: state read, session status, dice, notes, movement, owned token changes, extra movement, undo, and SSE.
...
- Every accepted shared-state mutation increments `battleMapVersion` and broadcasts a separately sanitized snapshot to each connected user.
