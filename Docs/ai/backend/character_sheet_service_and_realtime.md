...
- Attacks, tools, and auras are flat-field rows (see the schema's `CHARACTER_COLLECTIONS`), not nested objects, so they stay inside the existing `character.<collection>.<id>.<field>` patch grammar instead of requiring a second patch mechanism. Auras contain name, private description, public effect, radius in cells, palette color, and active state. Validation limits them to ten rows, a 1–24 cell radius, a 60-character name, and a 500-character effect; an older document without the collection loads with an empty list.
...
- `add`, `remove`, and `set` operations under `character.auras` trigger token projection. The server rereads the sheet and derives only `{ id, name, effect, radiusCells, color, active }` on the canonical player token; the description remains private. The dedicated aura switch endpoint applies a validated `set` patch to `active`, so it follows the same sheet version, SSE, debounce, and SQLite flush lifecycle as direct sheet editing.
...
- No mode is negotiated before a roll. Every target whose formula is `1d20 + modifier` (ability, saving throw, skill, tool, attack-to-hit) rolls **two independent `1d20`** in the same request and reports both raw faces in `rolls`; nothing is picked or discarded server-side. This matches the reference UX (Roll20): whoever reads the log decides which of the two counts (first for a normal roll, higher for advantage, lower for disadvantage). Hit dice and death saves stay a single roll.
- The `initiative` target is the exception (P0.8a): `POST /api/battle-map/rolls` routes it to the authoritative initiative roll (`server/initiative-roll.mjs`) instead of the resolver, after resolving the token linked to the sheet owner. It uses `character.initiativeRollMode` (`normal`/`advantage`/`disadvantage`, a closed-domain sheet field defaulting to `normal`, never inferred from the roster), produces one value, writes the tracker entry, and ignores the sheet's secret toggle. It is rejected in exploration or when the sheet has no token on the map; the resolver itself refuses the target if called directly.
