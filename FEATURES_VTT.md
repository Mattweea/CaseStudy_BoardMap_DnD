...
- **Aure come in Roll20, senza automazione:** nessun bonus viene applicato; l'app mostra l'area e ricorda l'effetto. Catalogo di riferimento: le aure dei manuali ufficiali 2014 (PHB, DMG, XGtE, TCoE, SCAG, VGtM).
  - *Definizione nella scheda*, più aure per personaggio: nome, raggio, colore, nota libera; chi la riceve (proprietario, alleati, nemici); promemoria nei tiri di (tiri salvezza per tutte o per singola caratteristica, salvezza contro morte, prove per tutte o per caratteristica, attacco, danno, iniziativa); promemoria a inizio turno, a fine turno e all'ingresso di una creatura; condizioni a cui dà immunità.
  - *Accensione* dal menu radiale del token, per ira, concentrazione e oggetto impugnato. *Sospensione automatica* quando il proprietario è Privo di sensi o Incapacitato; nessun bonus applicato, l'aura si spegne soltanto.
  - *Area* disegnata sulla griglia dal bordo del token, con la regola delle diagonali della partita. Tutte le aure sono visibili a tutti: il flag di visibilità attuale sparisce.
  - *Promemoria dei tiri:* una riga nella card del tiro, calcolata dal server; due aure con lo stesso nome compaiono una volta sola con «non si sommano». *Promemoria di turno e d'ingresso:* una riga di sistema nel log per chi controlla la creatura e per il Master. *Immunità:* un avviso nel menu radiale quando si applica la condizione, senza impedirla.
  - *Solo area e nota* per gli effetti che dipendono dal bersaglio di un attacco (Holy Aura, Totem del Lupo, Spirit Shroud), per le resistenze (riprese in `p0-8b` all'inserimento di `-N`), per le variazioni di velocità (solo promemoria, niente budget) e per le aree senza tiri (luce, Antimagic Field).
  - *Rinviate a P0.9:* aure dei mostri, aure ancorate a un punto spostabile della mappa (Spirit Totem) e aure a cono.
