...
export function DicePanel({ onRoll }: DicePanelProps) {
  ...
  // Vantaggio e svantaggio sono definiti solo sul tiro di un singolo d20: su qualunque altra
  ...
  return <section className="dice-panel" aria-label="Controlli dadi">
    ...
    <div className="dice-panel__actions"><span aria-live="polite">{count ? `${count}d${selectedDie}` : 'Scegli un dado'}</span><button type="button" className="primary-button" onClick={() => { if (!supportsRollMode) setMode('normal'); setIsConfigOpen(true); }} disabled={!count}>Tira</button></div>
